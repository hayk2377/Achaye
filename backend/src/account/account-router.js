...
router.delete("/profile", logGuard, async (req, res) => {
	const userId = req.session.userId;

	const result = await userInfoHandler.deletePreferences(userId);
	res.json(result);
});

//create profile
router.post("/profile", logGuard, profilePictureMulter, async (req, res) => {
